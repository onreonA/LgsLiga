  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-lg font-semibold text-gray-900">Konu Başarı Haritası</h3>
          <p className="text-sm text-gray-600">Konulara göre performans dağılımı</p>
        </div>
        ...
        {mockTopicData.map((topic, index) => (
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <h4 className="text-sm font-medium text-gray-900 truncate">{topic.name}</h4>
              <span className={`text-xs font-semibold ${getScoreTextColor(topic.score)}`}>
                {topic.score}%
              </span>
            </div>
            ...
            <div className="flex items-center justify-between">
              <span className={`text-xs px-2 py-1 rounded-full ${
                topic.difficulty === 'Kolay' ? 'bg-green-100 text-green-700' :
                topic.difficulty === 'Orta' ? 'bg-yellow-100 text-yellow-700' :
