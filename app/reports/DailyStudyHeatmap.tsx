...
export default function DailyStudyHeatmap() {
  const [selectedMetric, setSelectedMetric] = useState<'questions' | 'accuracy' | 'studyTime'>('questions');
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-lg font-semibold text-gray-900">Günlük Çalışma Aktivite Haritası</h3>
            ...
            return (
              <div
                ...
                title={`${date.getDate()} Ocak - ${
                  selectedMetric === 'questions' ? `${value} soru` :
                  selectedMetric === 'accuracy' ? `%${value} doğruluk` :
                  `${value} dakika`
                }`}
              >
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-600">
                {Math.floor(getDateInfo(selectedDate).studyTime / 60)}s {getDateInfo(selectedDate).studyTime % 60}d
              </div>
