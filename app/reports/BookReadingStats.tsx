...
const mockBookStats = [
  { book_name: 'Matematik Soru Bankası', total_pages: 450, pages_read: 283, reading_days: 12, completion_rate: 63, avg_daily_pages: 24 },
  { book_name: 'Türkçe Test Kitabı', total_pages: 320, pages_read: 260, reading_days: 13, completion_rate: 81, avg_daily_pages: 20 },
  { book_name: 'Fen Bilimleri Konu Anlatımı', total_pages: 280, pages_read: 147, reading_days: 7, completion_rate: 53, avg_daily_pages: 21 },
  { book_name: 'Sosyal Bilgiler Atlası', total_pages: 200, pages_read: 200, reading_days: 10, completion_rate: 100, avg_daily_pages: 20 },
  { book_name: 'İngilizce Kelime Kitabı', total_pages: 150, pages_read: 72, reading_days: 6, completion_rate: 48, avg_daily_pages: 12 }
];
...
export default function BookReadingStats() {
  ...
  const [selectedView, setSelectedView] = useState<'completion' | 'daily' | 'category'>('completion');
  ...
  const processBookData = (data: any[]) => {
    const bookGroups = data.reduce((acc, item) => {
    ...
    return Object.entries(bookGroups).map(([bookName, entries]) => {
      ...
      const completionRate = totalPages > 0 ? (totalPagesRead / totalPages) * 100 : 0;
      const avgDailyPages = readingDays > 0 ? Math.round(totalPagesRead / readingDays) : 0;
  ...
  const getCompletionData = () => {
    return bookStats.map((book, index) => ({
      name: book.book_name.length > 15 ? book.book_name.substring(0, 15) + '...' : book.book_name,
  ...
  const getCategoryData = () => {
    ...
    bookStats.forEach(book => {
      ...
      else if (book.book_name.toLowerCase().includes('sosyal')) category = 'Sosyal Bilgiler';
      else if (book.book_name.toLowerCase().includes('ingilizce')) category = 'İngilizce';
  ...
  const totalPagesRead = bookStats.reduce((sum, book) => sum + book.pages_read, 0);
  const totalBooks = bookStats.length;
  const completedBooks = bookStats.filter(book => book.completion_rate >= 100).length;
  const avgCompletion = bookStats.length > 0 
    ? Math.round(bookStats.reduce((sum, book) => sum + book.completion_rate, 0) / bookStats.length)
    : 0;
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-lg font-semibold text-gray-900">Kitap Okuma İstatistikleri</h3>
          <p className="text-sm text-gray-600">Kitap okuma alışkanlıklarınızın detaylı analizi</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-amber-50 rounded-xl p-4 text-center">
          <div className="text-2xl font-bold text-amber-600">{avgCompletion}%</div>
            ...
            {bookStats.map((book, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                <div className="flex items-center space-x-3">
                  ...
                  ></div>
                  <div>
                    <div className="font-medium text-gray-900">{book.book_name}</div>
                ...
                <div className="text-right">
                  <div className="font-bold text-gray-900">{book.completion_rate.toFixed(1)}%</div>
                  <div className="text-sm text-gray-600">{book.reading_days} gün</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {selectedView === 'daily' && (
        <div className="space-y-6">
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={readingHabits} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-green-50 rounded-xl p-4">
              <div className="flex items-center space-x-2 mb-2">
                ...
                <span className="text-sm font-medium text-green-700">En Verimli Gün</span>
              </div>
            ...
            <div className="bg-blue-50 rounded-xl p-4">
              <div className="flex items-center space-x-2 mb-2">
                ...
                <span className="text-sm font-medium text-blue-700">Haftalık Ortalama</span>
              </div>
              ...
              <div key={index} className="border border-gray-200 rounded-xl p-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="text-center">
                    <div className="text-lg font-bold text-gray-900">{category.pages}</div>
                    <div className="text-xs text-gray-600">Sayfa</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold text-gray-900">{category.books}</div>
