...
export default function WeakTopicsCard() {
  const getSubjectColor = (subject: string) => {
    switch (subject) {
      case 'Matematik': return 'bg-red-500';
      case 'Türkçe': return 'bg-orange-500';
      case 'Fen Bilimleri': return 'bg-yellow-500';
  ...
  const getUrgencyColor = (urgency: string) => {
    switch (urgency) {
      case 'Acil': return 'bg-red-100 text-red-700 border-red-200';
      case 'Önemli': return 'bg-orange-100 text-orange-700 border-orange-200';
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-lg font-semibold text-gray-900">Zayıf Kazanımlar</h3>
          <p className="text-sm text-gray-600">Acil çalışma gereken konular (50% altı)</p>
        </div>
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  <div className="flex items-center space-x-3 mb-2">
                    <div className={`w-3 h-3 ${getSubjectColor(topic.subject)} rounded-full`}></div>
                    <h4 className="font-semibold text-gray-900">{topic.topic}</h4>
                    <span className={`px-2 py-1 text-xs font-medium rounded-full border ${getUrgencyColor(urgency)}`}>
                      {urgency}
                    </span>
                  </div>
                ...
                <div className="text-right">
                  <div className="text-2xl font-bold text-red-600">{topic.score}%</div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-3">
                <div className="bg-gray-50 rounded-lg p-3">
                  <div className="text-lg font-semibold text-gray-900">{topic.correctAnswers}/{topic.totalQuestions}</div>
                  <div className="text-xs text-gray-600">Doğru/Toplam</div>
                </div>
                <div className="bg-gray-50 rounded-lg p-3">
                  <div className="text-lg font-semibold text-gray-900">{topic.lastStudied}</div>
