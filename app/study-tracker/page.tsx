...
export default function StudyTrackerPage() {
  const [activeTab, setActiveTab] = useState<"daily" | "exam" | "book">(
    "daily",
  );
  ...
  useEffect(() => {
    if (studyForm.subject) {
      const filtered = allTopics.filter(
        (topic) => topic.subject_id === studyForm.subject,
      );
  ...
  const loadBookList = async () => {
    try {
      ...
      const uniqueBooks = [
        ...new Set(data?.map((item) => item.book_name) || []),
      ];
  ...
  const handleStudySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(
        "Çalışma kaydedilirken bir hata oluştu: " + (error as Error).message,
      );
    }
  };
  ...
  const handleExamSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const totalCorrect =
        subjects.turkce.correct +
        subjects.tarih.correct +
        subjects.din.correct +
        subjects.ingilizce.correct +
        subjects.matematik.correct +
        subjects.fen.correct;

      const totalWrong =
        subjects.turkce.wrong +
        subjects.tarih.wrong +
        subjects.din.wrong +
        subjects.ingilizce.wrong +
        subjects.matematik.wrong +
        subjects.fen.wrong;

      const totalQuestions = Object.values(subjects).reduce(
        (sum, subj) => sum + subj.correct + subj.wrong + subj.empty,
        0,
      );
  ...
  const handleBookSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("book_reading").insert({
        user_id: user.id,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">
            Günlük çalışma, sınav ve kitap okuma verilerinizi kaydedin
          </p>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between">
            ...
            {activeTab === "daily" && studyForm.correctAnswers > 0 && (
              <div className="flex items-center space-x-2 text-sm">
                ...
                <span>•</span>
                <span>
                  +{Math.floor((studyForm.correctAnswers * 10) / 10)} Coin
                </span>
              </div>
            )}
          </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setStudyForm({ ...studyForm, date: e.target.value })
                  }
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setStudyForm({ ...studyForm, topic: e.target.value })
                  }
                  className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent pr-8"
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setStudyForm({ ...studyForm, source: e.target.value })
                  }
                  className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent pr-8"
                  required
                >
          ...
          <div className="mt-8 border-t border-gray-200 pt-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">
              Son Çözülen Sorular (Son 20 Kayıt)
            </h3>
                  ...
                  {studyHistory.length === 0 ? (
                    ...
                    studyHistory.map((item) => (
                      ...
                      >
                        ...
                        <td className="py-3 px-4 text-sm text-blue-600 text-center font-semibold">
                          {item.xp_earned}
                        </td>
                      </tr>
                    ))
      ...
      {activeTab === "exam" && (
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
          ...
          <form onSubmit={handleExamSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-4 gap-4 items-center bg-gray-100 p-4 rounded-lg border-2 border-gray-300">
                ...
                <div>
                  ...
                  <div className="w-full px-3 py-2 bg-white border-2 border-gray-400 rounded-lg font-bold text-green-700">
                    {Object.values(examForm.subjects).reduce(
                      (sum, s) => sum + s.correct,
                      0,
                    )}
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t">
              <div>
                ...
                <input
                  type="text"
                  value={Object.values(examForm.subjects)
                    .reduce((sum, s) => sum + (s.correct - s.wrong * 0.25), 0)
                    .toFixed(2)}
          ...
          <div className="mt-8 border-t border-gray-200 pt-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">
              Son Sınav Sonuçları (Son 20 Kayıt)
            </h3>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b-2 border-gray-300">
                    ...
                    <th className="text-center py-3 px-2 text-xs font-semibold text-blue-700">
                      Türkçe Net
                    </th>
                    <th className="text-center py-3 px-2 text-xs font-semibold text-amber-700">
                      Tarih Net
                    </th>
                    <th className="text-center py-3 px-2 text-xs font-semibold text-green-700">
                      Din Net
                    </th>
                    <th className="text-center py-3 px-2 text-xs font-semibold text-purple-700">
                      İng Net
                    </th>
                    <th className="text-center py-3 px-2 text-xs font-semibold text-red-700">
                      Mat Net
                    </th>
                    <th className="text-center py-3 px-2 text-xs font-semibold text-teal-700">
                      Fen Net
                    </th>
                    <th className="text-center py-3 px-2 text-xs font-bold text-gray-900 bg-gray-100">
                      Toplam Net
                    </th>
                    <th className="text-center py-3 px-3 text-xs font-bold text-gray-900 bg-gray-100">
                      Puan
                    </th>
                  </tr>
                </thead>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
          <div className="flex items-center mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                Günlük kitap okuma ilerlemenizi kaydedin
              </p>
            </div>
          </div>
              ...
              <div className="bg-gray-50 rounded-xl p-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm text-gray-600">
                    {Math.round(
                      (bookForm.pagesReadToday / bookForm.totalPages) * 100,
                    )}
                    %
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-3">
                  <div
                    ...
                    style={{
                      width: `${Math.min((bookForm.pagesReadToday / bookForm.totalPages) * 100, 100)}%`,
                    }}
                  ></div>
                </div>
          ...
          <div className="mt-8 border-t border-gray-200 pt-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">
              Kitap Okuma Geçmişi (Son 20 Kayıt)
            </h3>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  ...
                  {bookHistory.length === 0 ? (
                    ...
                    bookHistory.map((item) => (
                      ...
                      >
                        ...
                        <td className="py-3 px-4 text-sm text-green-600 text-center font-medium">
                          {item.pages_read_today}
                        </td>
                        <td className="py-3 px-4 text-sm text-gray-600 text-center">
                          {item.remaining_pages}
                        </td>
