...
const reminderMessages = [
  ...
  },
  {
    ...
    message: 'Gece yarısına 2 saat var! Serini kaybetme, hemen küçük bir çalışma başlat!',
    message_type: 'urgent',
    trigger_time: '22:00',
    is_active: true
  }
...
export default function StreakMessages() {
  ...
  const handleAddMessage = () => {
    if (newMessage.title.trim() && newMessage.message.trim()) {
      ...
      setNewMessage({ streak_days: 7, title: '', message: '', message_type: 'achievement' });
  ...
  const toggleMessage = (messageId: string, isReminder = false) => {
    if (isReminder) {
      setReminders(reminders.map(msg => 
        msg.id === messageId ? { ...msg, is_active: !msg.is_active } : msg
      ));
    } else {
      setStreakMessages(streakMessages.map(msg => 
        msg.id === messageId ? { ...msg, is_active: !msg.is_active } : msg
      ));
    }
  };
  ...
  const getMessageTypeColor = (type: string) => {
    switch (type) {
      ...
      case 'motivation': return 'bg-blue-100 text-blue-700 border-blue-200';
      case 'encouragement': return 'bg-purple-100 text-purple-700 border-purple-200';
  ...
  const getMessageTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'encouragement': return 'ri-thumb-up-line';
      case 'reminder': return 'ri-alarm-line';
      case 'urgent': return 'ri-alert-line';
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-lg font-semibold text-gray-900">Seri Mesajları</h3>
            <p className="text-sm text-gray-600">Çalışma serilerinde gösterilecek motivasyon mesajları</p>
          </div>
          <button
          ...
          <div className="bg-gray-50 rounded-xl p-4 mb-4">
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Seri Gün Sayısı</label>
                  <input
                    type="number"
                    value={newMessage.streak_days}
                    onChange={(e) => setNewMessage({ ...newMessage, streak_days: parseInt(e.target.value) || 0 })}
                    min="0"
        ...
        {activeTab === 'streaks' && (
          <div className="space-y-4">
            {streakMessages.map((message) => (
              <div key={message.id} className={`p-4 rounded-xl border ${message.is_active ? 'bg-white shadow-sm' : 'bg-gray-50 opacity-60'}`}>
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center space-x-3">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center border ${getMessageTypeColor(message.message_type)}`}>
                      <i className={`${getMessageTypeIcon(message.message_type)} text-sm`}></i>
                    </div>
                    <div>
        ...
        {activeTab === 'reminders' && (
          <div className="space-y-4">
            {reminders.map((reminder) => (
              <div key={reminder.id} className={`p-4 rounded-xl border ${reminder.is_active ? 'bg-white shadow-sm' : 'bg-gray-50 opacity-60'}`}>
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center space-x-3">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center border ${getMessageTypeColor(reminder.message_type)}`}>
                      <i className={`${getMessageTypeIcon(reminder.message_type)} text-sm`}></i>
                    </div>
                    <div>
                      <h4 className="font-semibold text-gray-900">{reminder.title}</h4>
                      <span className="text-xs text-gray-500">Saat: {reminder.trigger_time}</span>
                    </div>
                  </div>
                  <button
