...
const mockMessages = [
  ...FAMILY_MESSAGES.support.slice(0, 3).map((msg, index) => ({
    ...
    created_at: new Date(Date.now() - (index + 1) * 24 * 60 * 60 * 1000)
      .toISOString()
      .split("T")[0],
    is_active: true,
  })),
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-gray-600">
              Ailenin sana olan desteğini hissettiren mesajlar
            </p>
          </div>
          <button
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="text-xs text-gray-500 mt-1">
                  {newMessage.message.length}/500 karakter
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
        ...
        <div className="space-y-4">
          {messages.map((message) => (
            <div
              key={message.id}
              className={`p-4 rounded-xl border-l-4 ${message.is_active ? "bg-white border-l-pink-500 shadow-sm" : "bg-gray-50 border-l-gray-300 opacity-60"}`}
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center space-x-3">
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center ${getTypeColor(message.message_type)}`}
                  >
                    <i
                      className={`${getTypeIcon(message.message_type)} text-sm`}
                    ></i>
                  </div>
                  <div>
                    ...
                    <div className="flex items-center space-x-2">
                      <span
                        className={`px-2 py-0.5 text-xs font-medium rounded-full ${getTypeColor(message.message_type)}`}
                      >
