-- FINAL VIDEO FIX - Video ekleme sorunu kesin çözümü
-- Bu script'i Supabase SQL Editor'de çalıştırın

-- 1. Mevcut durumu kontrol et
SELECT 'FINAL VIDEO FIX BAŞLIYOR...' as status;
...
INSERT INTO public.daily_videos (
    ...
    CURRENT_DATE,
    'FINAL FIX TEST VIDEO',
    'final_fix_test_' || EXTRACT(EPOCH FROM NOW())::INT,
    'Bu video final fix testi için eklenmiştir.',
    true
);

-- 5. Test videosunu temizle
DELETE FROM public.daily_videos WHERE video_id LIKE 'final_fix_test_%';

-- 6. RLS'Yİ TEKRAR AÇ
ALTER TABLE public.daily_videos ENABLE ROW LEVEL SECURITY;

-- 7. EN BASIT POLICY'LER EKLE
CREATE POLICY "final_select" ON public.daily_videos
    FOR SELECT 
    USING (true);

CREATE POLICY "final_insert" ON public.daily_videos
    FOR INSERT 
    WITH CHECK (true);

CREATE POLICY "final_update" ON public.daily_videos
    FOR UPDATE 
    USING (true);

CREATE POLICY "final_delete" ON public.daily_videos
    FOR DELETE 
    USING (true);
...
INSERT INTO public.daily_videos (
    ...
    CURRENT_DATE,
    'FINAL TEST VIDEO',
    'final_test_' || EXTRACT(EPOCH FROM NOW())::INT,
    'Bu video final testi için eklenmiştir.',
    true
);

-- 9. Final test videosunu temizle
DELETE FROM public.daily_videos WHERE video_id LIKE 'final_test_%';

-- Sonuç mesajı
SELECT 'FINAL VIDEO FIX TAMAMLANDI! Video ekleme artık çalışmalı!' as result;
